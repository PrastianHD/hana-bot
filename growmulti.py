...
def load_proxies_from_file():
    try:
        ...
        print(Fore.RED + Style.BRIGHT + "File 'proxies.txt' tidak ditemukan." + Style.RESET_ALL)
        exit()
    ...
    while True:
        for index, account in enumerate(accounts):
            if index >= len(proxies):
                print_error(f"Not enough proxies for {len(accounts)} accounts.")
                return
            ...
            try:
                ...
                print_error(f"Error refreshing token for account {account['name']}: {e}")
